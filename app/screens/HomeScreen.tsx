  ...
  return (
    <>
     ...
     />
        ...
        <Pressable style={styles.bucketModalBackdrop} onPress={() => setActionTask(null)}>
          <Pressable style={[styles.taskActionCard, { backgroundColor: popupSurface, borderColor: popupBorder }]} onPress={(event) => event.stopPropagation()}>
            ...
            <View style={styles.taskActionGrid}>
              ...
              <Pressable style={[styles.taskActionOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { const target = actionTask; setActionTask(null); if (target) setTimeout(() => setPostponeTask(target), 0); }}><Text style={[styles.taskActionIcon, { color: popupAccent }]}>→</Text><View><Text style={[styles.taskActionLabel, { color: popupText }]}>延期する</Text><Text style={[styles.taskActionSubcopy, { color: popupMuted }]}>明日・週末・日付を指定</Text></View></Pressable>
            ...
            <View style={styles.taskActionGrid}>
              {actionTask?.isRoutine === true && <Pressable style={[styles.taskActionOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { if (actionTask?.isRoutine === true) onSkip(actionTask.id); setActionTask(null); }}><Text style={[styles.taskActionIcon, { color: popupAccent }]}>☾</Text><Text style={[styles.taskActionLabel, { color: popupText }]}>今日はスキップ</Text></Pressable>}
              <Pressable style={[styles.taskActionOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { const target = actionTask; setActionTask(null); if (target) setTimeout(() => setListTask(target), 0); }}><Text style={[styles.taskActionIcon, { color: popupAccent }]}>☷</Text><Text style={[styles.taskActionLabel, { color: popupText }]}>リスト・メモ{actionTask?.listItems?.length ? ` ${actionTask.listItems.length}件` : ''}</Text></Pressable>
              <Pressable style={[styles.taskActionOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { const target = actionTask; setActionTask(null); if (target) setTimeout(() => setBucketTask(target), 0); }}><Text style={[styles.taskActionIcon, { color: popupAccent }]}>⇄</Text><View><Text style={[styles.taskActionLabel, { color: popupText }]}>振り分ける</Text><Text style={[styles.taskActionSubcopy, { color: popupMuted }]}>今やる・あとで・待ち</Text></View></Pressable>
            </View>
      ...
      <Modal visible={Boolean(postponeTask)} transparent animationType="fade" onRequestClose={() => setPostponeTask(null)}>
        <Pressable style={styles.bucketModalBackdrop} onPress={() => setPostponeTask(null)}>
          <Pressable style={[styles.bucketModalCard, { backgroundColor: popupSurface, borderColor: popupBorder }]} onPress={(event) => event.stopPropagation()}>
            <Text style={[styles.bucketModalTitle, { color: popupText }]}>延期する</Text>
            <Text numberOfLines={1} style={[styles.bucketModalTask, { color: popupText }]}>{postponeTask?.title}</Text>
            <Pressable style={[styles.bucketModalOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { if (postponeTask) onPostpone(postponeTask.id, tomorrowKey); setPostponeTask(null); }}><View><Text style={[styles.bucketModalOptionTitle, { color: popupText }]}>明日</Text><Text style={[styles.bucketModalOptionCopy, { color: popupMuted }]}>翌日に延期</Text></View><Text style={[styles.bucketModalOptionCheck, { color: popupAccent }]}>›</Text></Pressable>
            <Pressable style={[styles.bucketModalOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { if (postponeTask) onPostpone(postponeTask.id, weekendKey); setPostponeTask(null); }}><View><Text style={[styles.bucketModalOptionTitle, { color: popupText }]}>週末</Text><Text style={[styles.bucketModalOptionCopy, { color: popupMuted }]}>次の土曜日へ延期</Text></View><Text style={[styles.bucketModalOptionCheck, { color: popupAccent }]}>›</Text></Pressable>
            <Pressable style={[styles.bucketModalOption, { backgroundColor: popupSoft, borderColor: popupBorder }]} onPress={() => { setPostponeDateTask(postponeTask); setPostponeTask(null); }}><View><Text style={[styles.bucketModalOptionTitle, { color: popupText }]}>日付を選ぶ</Text><Text style={[styles.bucketModalOptionCopy, { color: popupMuted }]}>カレンダーから指定</Text></View><Text style={[styles.bucketModalOptionCheck, { color: popupAccent }]}>›</Text></Pressable>
          </Pressable>
        </Pressable>
      </Modal>
