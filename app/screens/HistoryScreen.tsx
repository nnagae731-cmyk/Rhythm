...
export function HistoryScreen({ tasks, wishMonths, calendarMarks, onSetCalendarMark, recoveryHistory, focusSessions, departureCheckIns, departurePlans, behaviorEvents, completionIcon, designMode, chicPattern, chicPalette, planTier, onPremium, onSaveTemplate, onRestore, onSaveDailyReview, onSaveMonthlyReflectionCard, onUpdateReview, onDeleteReview, styles, helpers, components, previewMode = false, previewSearchQuery, previewJournal = false }: { tasks: Task[]; wishMonths: WishMonthMap; calendarMarks: CalendarMarks; onSetCalendarMark: (date: string, mark?: string) => void; recoveryHistory: RecoveryRecord[]; focusSessions: FocusSession[]; departureCheckIns: DepartureCheckIn[]; departurePlans: DeparturePlan[]; behaviorEvents: BehaviorEvent[]; completionIcon: string; designMode: ThemeMode; chicPattern: ChicPattern; chicPalette?: ChicThemePalette; planTier: PlanTier; onPremium: (featureId?: PremiumGuideFeatureId) => void; onSaveTemplate: (task: Task) => void; onRestore: (id: string) => void; onSaveDailyReview: (monthKey: string, draft: MonthlyReview) => void; onSaveMonthlyReflectionCard: (monthKey: string, card: MonthlyReflectionCard) => void; onUpdateReview: (monthKey: string, reviewKey: string, updates: Partial<MonthlyReview>) => void; onDeleteReview: (monthKey: string, reviewKey: string) => void; styles: any; helpers: any; components: any; previewMode?: boolean; previewSearchQuery?: string; previewJournal?: boolean }) {
  ...
  return (
    <>
      ...
      {previewMode && premiumHistory && searchQuery.trim() && <View style={[styles.historySearchModal, isDark && styles.darkSurface]}><View style={styles.historySearchModalHeader}><Text style={[styles.historySearchModalTitle, isDark && styles.darkBodyText]}>検索結果</Text><Text style={[styles.historySearchModalMeta, isDark && styles.darkMutedText]}>「{searchQuery}」・{selectedTasks.length}件</Text></View><ScrollView style={{ maxHeight: 240 }} nestedScrollEnabled>{selectedTasks.length === 0 ? <Text style={[styles.emptyCopy, isDark && styles.darkMutedText]}>検索結果がありません</Text> : selectedTasks.map((task) => <View key={task.id} style={[styles.historyTask, isDark && styles.darkHistoryTask]}><View style={[styles.historyIcon, isDark && styles.darkHistoryIcon]}><Text style={[styles.historyIconText, isDark && styles.darkAccentText]}>{completionIcon}</Text></View><View style={{ flex: 1 }}><Text style={[styles.taskTitle, isDark && styles.darkBodyText]}>{task.title}</Text><Text style={[styles.taskMeta, isDark && styles.darkMutedText]}>{task.category}・{task.completedAt ? dateKey(task.completedAt).replaceAll('-', '.') : ''}</Text></View></View>)}</ScrollView></View>}
      {previewJournal && premiumHistory && <View style={[styles.journalEditor, designMode === 'minimal' && styles.journalEditorMinimal, isDark && styles.darkJournalEditor, designMode === 'chic' && chicPalette && { backgroundColor: chicPalette.cardSurface, borderColor: chicPalette.border }]}><View style={styles.journalEditorHeader}><View><Text style={[styles.journalEditorTitle, isDark && styles.darkBodyText]}>今日の記録</Text><Text style={[styles.journalEditorDate, isDark && styles.darkAccentText]}>{selectedKey.replaceAll('-', '.')}</Text></View><Text style={[styles.journalEditorCount, isDark && styles.darkAccentText]}>写真 0 / {maxJournalPhotos}</Text></View><View style={styles.journalPhotoRow}><Pressable style={[styles.journalPhotoAdd, isDark && styles.darkJournalPhotoAdd]}><Text style={[styles.journalPhotoAddIcon, isDark && styles.darkAccentText]}>＋</Text><Text style={[styles.journalPhotoAddText, isDark && styles.darkAccentText]}>写真を追加</Text></Pressable></View><TextInput value="" editable={false} placeholder="今日のひとこと" placeholderTextColor={isDark ? '#8F9BB0' : '#A29DAA'} style={[styles.journalInput, isDark && styles.darkInput]} /><TextInput value="" editable={false} placeholder="今日のことを少し残す" placeholderTextColor={isDark ? '#8F9BB0' : '#A29DAA'} multiline style={[styles.journalInput, styles.journalMemo, isDark && styles.darkInput]} /><View style={styles.journalSatisfactionRow}>{[1, 2, 3, 4, 5].map((value) => <View key={value} style={[styles.journalSatisfaction, isDark && styles.darkJournalSatisfaction]}><Text style={[styles.journalSatisfactionText, isDark && styles.darkMutedText]}>{value}</Text></View>)}<Text style={[styles.journalSatisfactionLabel, isDark && styles.darkAccentText]}>満足度</Text></View><Pressable style={styles.journalSaveButton}><Text style={styles.journalSaveButtonText}>保存する</Text></Pressable></View>}
      ...
      <View style={[styles.calendarCard, designMode === 'minimal' && styles.calendarCardMinimal, isDark && styles.darkCalendarCard, designMode === 'chic' && chicPalette && { backgroundColor: chicPalette.cardSurface, borderColor: chicPalette.border }]}>
        <View style={[styles.calendarHeader, isDark && styles.darkCalendarHeader]}>
          ...
          <View style={styles.calendarSummaryActions}>
            <Pressable onPress={openMonthlyReflection} style={[styles.monthlyReflectionTrigger, isDark && styles.darkMonthlyReflectionTrigger, designMode === 'chic' && chicPalette && { borderColor: chicPalette.accent, backgroundColor: chicPalette.accentSoft }]} accessibilityRole="button">
              <Text style={[styles.monthlyReflectionTriggerText, isDark && styles.darkAccentText, designMode === 'chic' && chicPalette && { color: chicPalette.accentStrong }]}>今月を振り返る</Text>
            </Pressable>
      ...
      {!premiumHistory && <Pressable style={[styles.guideCard, premiumGuideCard]} onPress={() => onPremium('records')}>
        <View><Text style={[styles.guideCardTitle, premiumGuideTitle]}>今日の記録</Text><Text style={[styles.guideCardCopy, premiumGuideCopy]}>Premiumで日付ごとの写真・ひとこと・メモを残せます</Text></View><Text style={[styles.guideCardArrow, premiumGuideAccent]}>›</Text>
      </Pressable>}
