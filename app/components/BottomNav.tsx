      ...
      {items.map((item) => {
        ...
        return (
          <Pressable key={item.id} style={styles.navItem} onPress={() => onChange(item.id)}>
            <Text style={[styles.navIcon, { color: active ? theme.colors.primaryAccent : designMode === 'dark' ? '#5E43C6' : theme.colors.secondaryText }]}>{item.icon}</Text>
            <Text style={[styles.navLabel, { color: active ? theme.colors.primaryAccent : designMode === 'dark' ? '#5E43C6' : theme.colors.secondaryText }]}>{item.label}</Text>
          </Pressable>
        );
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  bottomNavDark: { backgroundColor: '#191B20', borderTopColor: '#3A404A' },
