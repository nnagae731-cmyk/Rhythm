...
export function TaskModal({ visible, task, templates, savedTemplates, designMode, chicPalette, planTier, onPremium, onClose, onOpenBulkAdd, onSave, readOnlyPreview = false, previewSection, guideOverlay, styles, helpers, components }: { visible: boolean; task?: Task; templates: string[]; savedTemplates: PremiumTaskTemplate[]; designMode: DesignMode; chicPalette?: ChicThemePalette; planTier: PlanTier; onPremium: (featureId?: PremiumGuideFeatureId) => void; onClose: () => void; onOpenBulkAdd?: () => void; onSave: (title: string, category: Category, priority: Priority, remindDate?: string, remindAt?: string, deadlineDate?: string, deadlineTime?: string, deadlineNotifyBefore?: number, navigationEnabled?: boolean, preparationMinutes?: number, travelMinutes?: number, bufferMinutes?: number, repeatRule?: RepeatRule, nudgeMode?: NudgeMode, scheduledDate?: string, scheduledTime?: string, endAt?: string, isRoutine?: boolean, subtasks?: Subtask[]) => void; readOnlyPreview?: boolean; previewSection?: 'savedTemplates'; guideOverlay?: React.ReactNode; styles: any; helpers: any; components: any }) {
  ...
  // Accent colors in the dark and Design themes can be light enough that
  // white text loses contrast. Keep the foreground paired with the active
  // theme token instead of relying on the legacy shared button style.
  const taskOnAccent = isChic && chicPalette ? chicPalette.onAccent : isDark ? theme.colors.screenBackground : '#FFFFFF';
      ...
      <Pressable style={[styles.modalBackdrop, readOnlyPreview && { flex: 0, backgroundColor: 'transparent', padding: 0 }]} onPress={closeForm}>
        <Pressable style={[styles.modalSheet, { backgroundColor: theme.colors.screenBackground, borderRadius: theme.radius.modal }, readOnlyPreview && { maxHeight: undefined, borderRadius: 14 }]} onPress={(event) => event.stopPropagation()}>
          ...
          {(Boolean(task) || detailsOpen) && <View style={{ marginTop: 4 }}>
            ...
            {expandedSetting === 'time' && <View style={{ paddingVertical: 6 }}><Pressable onPress={toggleScheduledTimePicker} style={{ minHeight: 40, paddingHorizontal: 8, justifyContent: 'center' }}><Text style={{ color: theme.colors.primaryText, fontSize: 13 }}>開始時間 {scheduledTime || '指定なし'} ›</Text></Pressable><TextInput value={scheduledEndTime} onChangeText={setScheduledEndTime} placeholder="終了時間（任意）" placeholderTextColor={theme.colors.secondaryText} keyboardType="numbers-and-punctuation" style={[styles.modalInput, { marginTop: 4, minHeight: 40 }, isDark && styles.darkInput]} />{showScheduledTimePicker && <DateTimePicker value={dateForReminder(scheduledDate, scheduledTime || '09:00')} mode="time" display={Platform.OS === 'ios' ? 'spinner' : 'default'} themeVariant={isDark ? 'dark' : undefined} textColor={theme.colors.primaryText} accentColor={theme.colors.primaryAccent} onChange={(event: DateTimePickerEvent, selected) => { if (Platform.OS !== 'ios') setShowScheduledTimePicker(false); if (event.type === 'set' && selected) { setScheduledTime(formatLiveTime(selected)); setShowScheduledTimePicker(false); } }} />}</View>}
          ...
          <Pressable style={[styles.primaryButton, { backgroundColor: taskAccent, borderRadius: theme.radius.button }]} onPress={save}><Text style={[styles.primaryButtonText, { color: taskOnAccent }]}>{task ? '変更を保存' : '登録する'}</Text></Pressable>
