...
export function AffirmationSettingsCard({ affirmations, customTexts, designMode, chicPalette, planTier, onPremium, onSave, onDelete, onSaveCustomText, onDeleteCustomText, styles, compact = false, previewMode = false }: Props) {
  ...
  const managementContent = <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
    ...
    <Pressable disabled={!editing && affirmations.length >= 5} onPress={() => void submit()} style={{ minHeight: 46, marginTop: 10, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: !editing && affirmations.length >= 5 ? border : accent }}><Text style={{ color: !editing && affirmations.length >= 5 ? secondary : onAccent, fontWeight: '800' }}>{editing ? '保存' : '通知を追加'}</Text></Pressable>
