...
export function TravelAppsSettingsCard({ settings, onChange, planTier, designMode, chicPalette, onPremium, readOnlyPreview = false }: Props) {
  ...
  return <View style={{ borderWidth: 1, borderColor: colors.border, borderRadius: 18, padding: 16, backgroundColor: colors.surface, marginBottom: 14 }}>
    ...
    <Text style={{ color: colors.text, fontSize: 13, fontWeight: '800', marginTop: 18 }}>乗換・地図</Text>
    {normalized.apps.filter((app) => app.category === 'transit').map(renderRow)}
    {!readOnlyPreview && <Pressable onPress={() => startEditor()} style={{ borderWidth: 1, borderColor: colors.accent, borderRadius: 11, minHeight: 44, alignItems: 'center', justifyContent: 'center', marginTop: 14 }}><Text style={{ color: colors.accent, fontSize: 12, fontWeight: '900' }}>＋ アプリを追加</Text></Pressable>}
