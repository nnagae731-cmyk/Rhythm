...
const GUIDE_ITEMS: CaptureItem[] = ['todo', 'voice', 'todoComplete', 'completedTasks', 'taskBuckets', 'taskDetails', 'schedule', 'planRegistration', 'calendarImport', 'focus', 'analysis', 'routine', 'history', 'photoLog', 'wish', 'affirmation', 'recovery'].map((id) => ({ id, label: id, mode: 'guide' as const }));
const PREMIUM_ITEMS: CaptureItem[] = ['voice', 'focus_custom_duration', 'records', 'reflection', 'calendar', 'route', 'travel_apps', 'nudge', 'time', 'behavior', 'month', 'history', 'recovery', 'templates', 'wish', 'affirmation', 'photo_design'].map((id) => ({ id, label: id, mode: 'premium' as const }));
